...
# Base command
BASE_CMD="python script/run.py --config-dir=cfg/gym/finetune/hopper-v2 --config-name=ft_ppo_reflow_mlp"
...
NOISE_LEVEL_A=0.6
BASE_PARAMS="min_std=0.08 max_std=0.16 train.ent_coef=0.03 wandb.offline_mode=True model.noise_level_a=${NOISE_LEVEL_A}"
...
# Log file
LOG_FILE="$RESULTS_DIR/seed_search_hopperv2_$(date +%Y%m%d_%H%M%S).log"

echo "Starting seed search for hopper-v2 environment"
echo "Seeds: ${SEEDS[*]}"
